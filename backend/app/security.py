...
def issue_token() -> str:
    return f"gcn_{secrets.token_urlsafe(32)}"
...
def require_admin(supplied_token: Annotated[str | None, Header(alias="X-GoreeCloud-Admin-Token")] = None) -> None:
...
def require_scope(scope: str) -> Callable[..., TokenPrincipal]:
    def dependency(
        ...
        record = session.scalar(select(AccessToken).where(AccessToken.token_digest == token_digest(raw_token)))
        ...
        return TokenPrincipal(record.id, identity.id, identity.name, frozenset(scopes))
